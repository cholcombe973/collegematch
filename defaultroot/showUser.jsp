<%@ page import="java.sql.*,java.io.*,collegematch.*,java.util.*,javax.naming.*, javax.sql.*" %>
...
<%
	String username = "";
	try{
		PreparedStatement transformToUsername = con.prepareStatement("Select Username from users where id=?");
              	transformToUsername.setInt(1,Integer.parseInt(request.getParameter("user")));
		ResultSet rs = transformToUsername.executeQuery();
		if(rs.next())
			username = rs.getString(1);
		transformToUsername.close();
              	rs.close();
...
%>
  <div id="content">
	...
	<img src="picservlet?a=BINARYFILE&b=<%=request.getParameter("user")%>"/>
      <!--Picture loaded from the database -->
	...
	<%
		...
		long end2 = 0;
	try{
	      	PreparedStatement ownWords = con.prepareStatement("Select DescribeYourself,DescribeYourMatch from ownwords, users where ownwords.id = users.id and users.username=?",ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
		ownWords.setString(1,username);
              	ResultSet rs = ownWords.executeQuery();
		Clob c = null;
...
<%
	...
	<tr>
		...
		try{
			PreparedStatement ps = con.prepareStatement("Select EyeColor,HairColor,BodyType,BestFeature,Age,Height from appearance,users where appearance.id = users.id and users.username=?");
      ...
      <table width="640" border="1">
        ...
        <tr>
            <%
            try{
		PreparedStatement ps = con.prepareStatement("select FavHotSpots,FavThings,LastBookRead,Sign,Major,School, DoForFun from interests,users where interests.id = users.id and users.username=?");
		ps.setString(1,username);
		ResultSet rs = ps.executeQuery();
		if(rs.next()){
      ...
      <table width="640" border="1">
        ...
        <tr>
            <%
		try{
			PreparedStatement ps = con.prepareStatement("Select Exercise,Diet,Smoke,Drink,Major,CurrentLivingLocation from lifestyle,users where lifestyle.id = users.id and users.username=?");
			ps.setString(1,username);
			ResultSet rs = ps.executeQuery();
			if(rs.next()){
				buff.delete(0,buff.length());
      ...
      <table width="640" border="1">
        ...
        <tr>
            <%
            	try{
			PreparedStatement ps = con.prepareStatement("Select PublicDisplaysOfAffection,BodyPiercings,Boldness,Brainiacs,Candlelight,Dancing,Erotica,Flirting,LongHair,Money,Power,Sarcasm,SkinnyDipping,Tattoos,Thrills from turnons,users where turnons.id = users.id and users.username=?");
			ps.setString(1,username);
			ResultSet rs = ps.executeQuery();
                        if(rs.next()){
				buff.delete(0,buff.length());
      ...
      <table width="640" border="1">
        ...
        <tr>
            <%
		try{
			PreparedStatement ps = con.prepareStatement("Select Ethnicities,Faith,Languages from value,users where value.id = users.id and users.username=?");
			ps.setString(1,username);
                        ResultSet rs = ps.executeQuery();
			if(rs.next()){
				buff.delete(0,buff.length());
      ...
      <table width="640" border="1">
        ...
        <tr>
		<%
			out.println("<td bgcolor=\"#F7F7F7\" width=\"264\"><span class=\"cssGlobalSysText_LightGray\">Total Execution time:</span></td>");
			out.println("<td bgcolor=\"#F7F7F7\" width=\"360\"><span class=\"cssGlobalSysText_LightGray\">"+(System.currentTimeMillis() - start)+"</span></td>");
                %>
		</tr>
              </table>
      <table width="640" border="1">
        <tr>
          <td colspan="2"><a href="#" class="capsule">**Save any changes made**</a> </td>
        </tr>
        <tr>
	 <td>Save button here</td>
        </tr>
      </table>
    </div>
  </div>
